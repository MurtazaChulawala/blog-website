...
try {
  await mongoose.connect('mongodb+srv://'+user+':'+password+'@cluster0.smjnmtt.mongodb.net/BlogDB');
} catch (error) {
  console.log(error);
}
...
let post = mongoose.model('blog', schema); 
...
app.get("/posts/:postTitle",async (req,res)=>{
  ...
  let posts = await post.findOne({title:title});
      res.render("post.ejs",{
        title:posts.title,
